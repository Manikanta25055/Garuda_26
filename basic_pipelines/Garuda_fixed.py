...
def voice_assistant_loop(stop_event, current_user=None):
    ...
    # Try to find the USB microphone specifically
    mic_index = None
    try:
        # List all microphones
        mic_list = sr.Microphone.list_microphone_names()
        append_voice_log(f"Available microphones: {mic_list}", user_name=current_user)
        
        # Look for the USB microphone (jieli technology UACDemoV1.0)
        for i, mic_name in enumerate(mic_list):
            if "usb" in mic_name.lower() or "uacdemo" in mic_name.lower() or "jieli" in mic_name.lower():
                mic_index = i
                append_voice_log(f"Found USB microphone at index {i}: {mic_name}", user_name=current_user)
                break
        ...
        if mic_index is not None:
            ...
            mic = sr.Microphone()
            append_voice_log("Using default microphone (USB mic not found)", user_name=current_user)
            
        append_voice_log("Microphone initialized successfully.", user_name=current_user)
        
    except Exception as e:
        append_voice_log(f"Error accessing microphone: {str(e)}", user_name=current_user)
        return
    ...
    with mic as source:
        append_voice_log("Adjusting for ambient noise... Please wait.", user_name=current_user)
        recognizer.adjust_for_ambient_noise(source, duration=2)
        append_voice_log("Calibration complete. Ready to listen.", user_name=current_user)
    ...
    while not stop_event.is_set():
        try:
            ...
            try:
                append_voice_log("Processing speech...", user_name=current_user)
                user_input = recognizer.recognize_google(audio)
                append_voice_log(f"You said: {user_input}", user_name=current_user)
            except sr.UnknownValueError:
                append_voice_log("Could not understand audio. Please speak clearly.", user_name=current_user)
                continue
            ...
            append_voice_log(f"Error in voice processing: {str(e)}", user_name=current_user)
            continue
...
class UserDashboardGUI:
    def __init__(self, root, user_data, username="user"):
        ...
        self.user_prefs = {}
        self.activity_cards = {}
        self.detection_cards = {}  # Initialize detection cards dict
    ...
    def load_user_preferences(self):
        ...
        if self.username in USERS:
            self.user_prefs = USERS[self.username].get("preferences", {
                "sound_alerts": True,
                "visual_alerts": True,
                "email_alerts": True,
                "auto_scroll": True,
            ...
            if "preferences" not in USERS[self.username]:
                USERS[self.username]["preferences"] = self.user_prefs
    ...
    def create_detection_page(self):
        ...
        # Stats cards row
        stats_row = ModernFrame(page)
        stats_row.pack(fill=tk.X, padx=10, pady=5)
        
        self.detection_cards = {}
        card_data = [
            ("total", "Total Detections", "0"),
            ("scissors", "Scissors Count", "0"),
            ("persons", "Persons Count", "0"),
            ("last_time", "Last Detection", "Never")
        ]
        
        for i, (key, label, default) in enumerate(card_data):
            card = ModernFrame(stats_row, bg=COLORS["bg_tertiary"], border=1)
            card.grid(row=0, column=i, padx=5, pady=5, sticky="ew")
            stats_row.columnconfigure(i, weight=1)
            
            card_label = ModernLabel(card, text=label, size=9, style="secondary")
            card_label.pack(pady=(10, 5))
            
            card_value = ModernLabel(card, text=default, size=12, weight="bold")
            card_value.pack(pady=(0, 10))
            
            self.detection_cards[key] = card_value
        ...
        self.system_info_label = ModernLabel(info_bar, text="System ready. Monitoring for detections...", size=10, style="secondary")
        ...
        det_header = ModernFrame(det_panel, bg=COLORS["bg_secondary"])
        det_header.pack(fill=tk.X)
        
        det_label = ModernLabel(det_header, text="Live Detection Feed", size=12, weight="bold")
        det_label.pack(side=tk.LEFT, padx=10, pady=5)
        
        self.det_fps = ModernLabel(det_header, text="FPS: --", size=9, style="secondary")
        self.det_fps.pack(side=tk.RIGHT, padx=10, pady=5)
        
        self.detection_text = ModernText(det_panel, height=15)
        self.detection_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        ...
        console_header = ModernFrame(console_panel, bg=COLORS["bg_secondary"])
        console_header.pack(fill=tk.X)
        
        console_label = ModernLabel(console_header, text="System Console", size=12, weight="bold")
        console_label.pack(side=tk.LEFT, padx=10, pady=5)
        
        self.console_count = ModernLabel(console_header, text="0 events", size=9, style="secondary")
        self.console_count.pack(side=tk.RIGHT, padx=10, pady=5)
    ...
    def create_preferences_page(self):
        ...
        self.pref_sound = tk.BooleanVar(value=self.user_prefs.get("sound_alerts", True))
        tk.Checkbutton(
        ...
        self.pref_visual = tk.BooleanVar(value=self.user_prefs.get("visual_alerts", True))
        tk.Checkbutton(
        ...
        self.pref_email_alerts = tk.BooleanVar(value=self.user_prefs.get("email_alerts", True))
        tk.Checkbutton(
        ...
        self.pref_auto_scroll = tk.BooleanVar(value=self.user_prefs.get("auto_scroll", True))
        tk.Checkbutton(
        ...
        # Interface theme
        theme_frame = ModernFrame(form_frame, bg=COLORS["bg_secondary"])
        theme_frame.pack(pady=10)
        
        ModernLabel(theme_frame, text="Interface Theme", size=12, weight="bold").pack(pady=5)
        
        self.pref_theme = tk.StringVar(value=self.user_prefs.get("theme", "dark"))
        themes = [("Dark Mode", "dark"), ("Light Mode", "light")]
        for text, value in themes:
            tk.Radiobutton(
                theme_frame, text=text, variable=self.pref_theme, value=value,
                bg=COLORS["bg_secondary"], fg=COLORS["text_primary"],
                activebackground=COLORS["bg_secondary"], selectcolor=COLORS["bg_primary"],
                font=("Arial", 10)
    ...
    def create_narada_page(self):
        ...
        # Status card
        status_card = ModernFrame(page, bg=COLORS["bg_tertiary"], border=1)
        status_card.pack(fill=tk.X, padx=10, pady=5)
        
        status_container = ModernFrame(status_card, bg=COLORS["bg_tertiary"])
        status_container.pack(pady=15)
        
        # Listening indicator with animation effect
        self.listening_indicator = ModernLabel(status_container, text="●", size=30)
        self.listening_indicator.config(fg=COLORS["error"])
        self.listening_indicator.pack()
        
        self.lbl_status = ModernLabel(status_container, text="Status: Not Listening", size=14, weight="bold")
        ...
        # Quick stats
        stats_frame = ModernFrame(control_frame, bg=COLORS["bg_secondary"])
        stats_frame.pack(side=tk.RIGHT, padx=20)
        
        self.voice_stats_label = ModernLabel(stats_frame, text="Commands: 0 | Responses: 0", size=10, style="secondary")
        self.voice_stats_label.pack()
        ...
        voice_header = ModernFrame(voice_panel, bg=COLORS["bg_secondary"])
        voice_header.pack(fill=tk.X)
        
        voice_label = ModernLabel(voice_header, text="Voice Log", size=12, weight="bold")
        voice_label.pack(side=tk.LEFT, padx=10, pady=5)
        
        self.voice_log_count = ModernLabel(voice_header, text="0 entries", size=9, style="secondary")
        self.voice_log_count.pack(side=tk.RIGHT, padx=10, pady=5)
        
        self.voice_text = ModernText(voice_panel, height=10, bg="#1f1f3a")
        self.voice_text.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        ...
        response_header = ModernFrame(response_panel, bg=COLORS["bg_secondary"])
        response_header.pack(fill=tk.X)
        
        response_label = ModernLabel(response_header, text="Narada Responses", size=12, weight="bold")
        response_label.pack(side=tk.LEFT, padx=10, pady=5)
        
        self.response_count = ModernLabel(response_header, text="0 responses", size=9, style="secondary")
        self.response_count.pack(side=tk.RIGHT, padx=10, pady=5)
    ...
    def save_preferences(self):
        ...
        self.user_prefs = {
            "sound_alerts": self.pref_sound.get(),
            "visual_alerts": self.pref_visual.get(),
            "email_alerts": self.pref_email_alerts.get(),
            "auto_scroll": self.pref_auto_scroll.get(),
            "theme": self.pref_theme.get()
        }
        ...
        if self.user_prefs["theme"] == "light":
            messagebox.showinfo("Theme Change", "Light theme will be applied on next login.")
        
        log_system_update(f"User {self.username} saved preferences: {self.user_prefs}")
    ...
    def update_gui(self):
        ...
        if self.pages["detection"].winfo_ismapped():
            ...
            self.detection_cards["total"].config(text=str(detection_stats["total_detections"]))
            self.detection_cards["scissors"].config(text=str(detection_stats["scissors_count"]))
            self.detection_cards["persons"].config(text=str(detection_stats["person_count"]))
            
            if detection_stats["last_detection_time"]:
                last_time = datetime.datetime.fromisoformat(detection_stats["last_detection_time"])
                time_diff = datetime.datetime.now() - last_time
                if time_diff.seconds < 60:
                    self.detection_cards["last_time"].config(text=f"{time_diff.seconds}s ago")
                elif time_diff.seconds < 3600:
                    self.detection_cards["last_time"].config(text=f"{time_diff.seconds // 60}m ago")
                else:
                    self.detection_cards["last_time"].config(text=last_time.strftime("%I:%M %p"))
            else:
                self.detection_cards["last_time"].config(text="Never")
            
            # Update system info
            quiet_status = "Quiet hours active" if is_quiet_hours() else ""
            sensitivity = f"Sensitivity: {system_settings['detection_sensitivity']}"
            self.system_info_label.config(text=f"System monitoring... {quiet_status} | {sensitivity}")
            
            # Update detection text with frame info
            if latest_detection_info:
                # Extract frame number for FPS calculation
                if "Frame:" in latest_detection_info:
                    frame_num = latest_detection_info.split("Frame:")[1].split("\n")[0].strip()
                    self.det_fps.config(text=f"Frame: {frame_num}")
            ...
            user_logs = []
            for log in system_updates_log[-50:]:
                # Filter logs to show only relevant ones for user
                if any(keyword in log.lower() for keyword in ["detection", "alert", "mode", "scissors", "person", "email sent", self.username.lower()]):
                    user_logs.append(log)
            
            if user_logs:
                update_text_widget(self.console_text, "\n".join(user_logs[-20:]))
                self.console_count.config(text=f"{len(user_logs)} events")
            else:
                update_text_widget(self.console_text, "System console ready.\nDetection events will appear here.")
                self.console_count.config(text="0 events")

        # Update Narada page
        if self.pages["narada"].winfo_ismapped():
            # Get user-specific logs
            user_voice_logs = []
            user_voice_responses = []
            
            # Simple approach - show recent logs from this session
            for log in voice_assistant_log[-50:]:
                user_voice_logs.append(log)
            
            for resp in voice_responses[-20:]:
                user_voice_responses.append(resp)
            
            # Update counts
            self.voice_log_count.config(text=f"{len(user_voice_logs)} entries")
            self.response_count.config(text=f"{len(user_voice_responses)} responses")
            self.voice_stats_label.config(text=f"Commands: {len(user_voice_logs)} | Responses: {len(user_voice_responses)}")
            
            update_text_widget(self.voice_text, "\n".join(user_voice_logs) if user_voice_logs else "Voice assistant log empty.\nStart Narada to begin.")
            ...
            if not self.voice_assistant_running:
                ...
                update_text_widget(self.response_text, "\n".join(user_voice_responses) if user_voice_responses else "Waiting for voice commands...")
        ...
        if self.pages["activity"].winfo_ismapped():
            ...
            if self.username in USERS:
                ...
                # Login history
                activity_text.append("=== LOGIN HISTORY ===")
                if history["logins"]:
                    for i, login in enumerate(history["logins"][-10:], 1):
                        activity_text.append(f"{i}. {login}")
                else:
                ...
                # Voice activity
                activity_text.append("=== RECENT VOICE COMMANDS ===")
                recent_commands = []
                for entry in history["narada_activity"]:
                    if "You said:" in entry:
                        recent_commands.append(entry)
                
                if recent_commands:
                    for cmd in recent_commands[-10:]:
                        activity_text.append(cmd)
                else:
                    activity_text.append("No voice commands recorded yet.")
                
                activity_text.append("")
                
                # System interactions
                activity_text.append("=== RECENT SYSTEM INTERACTIONS ===")
                interactions = []
                for log in system_updates_log:
                    if f"User {self.username}" in log or (self.username in log and "toggled" in log):
                        interactions.append(log)
                
                if interactions:
                    for interaction in interactions[-10:]:
                        activity_text.append(interaction)
                else:
                    activity_text.append("No system interactions recorded yet.")
    ...
    def animate_listening_indicator(self):
        """Animate the listening indicator when active."""
        if self.voice_assistant_running:
            current_size = self.listening_indicator.cget("font")[1]
            if current_size == 30:
                self.listening_indicator.config(font=("Arial", 28))
            else:
                self.listening_indicator.config(font=("Arial", 30))
