...
def log_system_update(message):
    """Append a system update with timestamp, e.g. [YYYY-MM-DD HH:MM:SS] Some update."""
...
##############################################################################
# VOICE ASSISTANT LOOP (Now stoppable via an Event)
##############################################################################
def voice_assistant_loop(stop_event):
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            response = None
            # Process a few example commands:
...
class SecurityDashboardGUI:
    def __init__(self, root, user_data):
        ...
        # --- Voice Assistant Modes Section (as before) ---
        ...
        # --- Voice Assistant Control Section ---
        self.voice_control_frame = ttk.Frame(self.voice_assistant_tab)
        self.voice_control_frame.pack(fill=tk.X, padx=10, pady=5)

        self.voice_button = ttk.Button(self.voice_control_frame, text="Start Voice Assistant", command=self.toggle_voice_assistant)
        ...
        # --- Voice Assistant Log Section ---
        self.voice_log_frame = ttk.LabelFrame(self.voice_assistant_tab, text="Voice Assistant Log")
        self.voice_log_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        self.voice_text = tk.Text(self.voice_log_frame, height=10, wrap="word", bg="lightyellow")
        self.voice_text.configure(state="disabled")
        self.voice_text.pack(fill=tk.BOTH, expand=True)

        # --- System Data Terminal Section ---
        self.system_data_frame = ttk.LabelFrame(self.voice_assistant_tab, text="System Data Terminal")
        self.system_data_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        self.system_text = tk.Text(self.system_data_frame, height=10, wrap="word", bg="lightblue")
        ...
        # Voice Assistant thread and stop event initialization
    ...
    def toggle_voice_assistant(self):
        if not self.voice_assistant_running:
            ...
            self.voice_button.config(text="Stop Voice Assistant")
            self.voice_status_label.config(text="Status: Listening...")
            append_voice_log("Voice Assistant started via GUI.")
        else:
            # Stop the voice assistant thread
            if self.voice_stop_event:
                self.voice_stop_event.set()
            self.voice_assistant_running = False
            self.voice_button.config(text="Start Voice Assistant")
            self.voice_status_label.config(text="Status: Not Listening")
            append_voice_log("Voice Assistant stopped via GUI.")
    ...
    def update_voice_assistant_tab(self):
        ...
        for msg in system_updates_log[-50:]:
            self.system_text.insert(tk.END, msg + "\n")
        self.system_text.configure(state="disabled")
...
if __name__ == "__main__":
    ...
    # Note: The Voice Assistant thread is now started/stopped via the GUI button.
