...
import serial  # (No longer used; can be removed)
import re
...
# Import requests for LLM HTTP calls
import requests
...
MODE_NIGHT = False       # High priority mode: LED/buzzer alerts last 10 sec and email subject is modified

# Global logs for Narada
voice_assistant_log = []   # General log from Narada (LLM integration, status, etc.)
voice_responses = []       # LLM responses (to be displayed in the Responses section)
...
# A global container for system updates (used in AI Detections tab)
system_updates_log = []

# This global will be assigned to the GUI instance so that the LLM thread can update checkbuttons.
dashboard_gui = None

# A simple string for detection info (updated by the pipeline callback)
...
def send_email_alert():
    ...
    # Skip email if Email Off or Idle mode is active
...
##############################################################################
# VOICE ASSISTANT LOOP (Using Ollama/TinyLlama via a local API)
##############################################################################
def voice_assistant_loop(stop_event):
    recognizer = sr.Recognizer()
    try:
        mic = sr.Microphone()  # Use your USB microphone
    ...
    while not stop_event.is_set():
        ...
        # Prepare prompt for the local LLM (Ollama using tinyllama)
        payload = {"prompt": user_input, "model": "tinyllama"}
        try:
            # Assume Ollama is running locally on port 11434; adjust if needed
            r = requests.post("http://localhost:11434/api/generate", json=payload, timeout=5)
            if r.status_code == 200:
                llm_response = r.json().get("response", "").strip()
            else:
                llm_response = f"Error: LLM returned status code {r.status_code}"
        except Exception as e:
            llm_response = f"Error contacting LLM: {str(e)}"

        # Append the LLM response to both logs and responses
        append_voice_log("Narada (LLM) response: " + llm_response)
        append_voice_response(llm_response)

        # --- Parse LLM response for mode instructions ---
        # (A simple parser; you can refine this logic as needed.)
        lower_resp = llm_response.lower()
        global MODE_DND, MODE_EMAIL_OFF, MODE_IDLE, MODE_NIGHT
        updated = False
        if "activate dnd" in lower_resp:
            MODE_DND = True
            updated = True
        if "deactivate dnd" in lower_resp:
            MODE_DND = False
            updated = True
        if "activate email off" in lower_resp:
            MODE_EMAIL_OFF = True
            updated = True
        if "deactivate email off" in lower_resp:
            MODE_EMAIL_OFF = False
            updated = True
        if "activate idle" in lower_resp:
            MODE_IDLE = True
            updated = True
        if "deactivate idle" in lower_resp:
            MODE_IDLE = False
            updated = True
        if "activate night mode" in lower_resp:
            MODE_NIGHT = True
            updated = True
        if "deactivate night mode" in lower_resp:
            MODE_NIGHT = False
            updated = True
        # If any mode was updated, also update the GUI checkbuttons:
        if updated and dashboard_gui:
...
class SecurityDashboardGUI:
    def __init__(self, root, user_data):
        ...
        # Narada tab now uses a container frame that occupies 50% of the tab’s height.
        # Within this container, the upper 35% is for Narada Logs and the lower 15% is for Responses.
        ...
        # Upper part: Narada Log (35% of 700 = ~245 pixels)
        ...
        # Lower part: Responses (15% of 700 = ~105 pixels) with white background and grey text
        ...
        # Narada Control Section (above the container) for starting/stopping Narada
    ...
    def toggle_voice_assistant(self):
        if not self.voice_assistant_running:
            # Start Narada (voice assistant) thread
...
if __name__ == "__main__":
    ...
    # Assign the dashboard instance to a global variable for use by the LLM thread.
    root = tk.Tk()
