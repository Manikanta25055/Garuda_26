...
MODE_DND = False         # Do Not Disturb (disables LED/buzzer)
MODE_EMAIL_OFF = False   # Email notifications off
MODE_IDLE = False        # All alerts disabled
MODE_NIGHT = False       # High priority mode (alerts last 10 sec)

# Global logs
system_updates_log = []    # System updates and events
voice_assistant_log = []   # Narada log messages (listening, recognized speech, etc.)
voice_responses = []       # Narada responses

# This variable will later be assigned to the dashboard instance so that the voice loop can update UI checkbuttons.
dashboard_gui = None
...
def log_system_update(message):
    """Append a message with a timestamp to the system updates log."""
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    system_updates_log.append(f"[{timestamp}] {message}")

def append_voice_log(message):
    """Append a message with a timestamp to the Narada log."""
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    voice_assistant_log.append(f"[{timestamp}] {message}")

def append_voice_response(message):
    """Append a message with a timestamp to the Narada responses."""
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def take_snapshot(frame, frame_count):
    # (Placeholder: Implement snapshot functionality if needed)
    pass
...
# ---------- Pipeline Class (Integrated Hailo/GStreamer) ----------
...
class UserDashboard(tk.Frame):
    ...
    def update_narada_tab(self):
        from common import voice_assistant_log, voice_responses
    ...
    def update_gui(self):
        ...
        from common import system_updates_log
...
if __name__ == "__main__":
    ...
    # Parse any command-line arguments if needed (for pipeline options)
