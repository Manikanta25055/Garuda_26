...
MODE_NIGHT = False       # High priority mode: alerts last 10 sec instead of 5, and email subjects are prefixed
...
# This variable will be set to the GUI instance so that the voice assistant thread can update checkbuttons.
dashboard_gui = None
...
def send_email_alert():
    ...
    # Skip sending if Email Off or Idle mode is active.
...
##############################################################################
# VOICE ASSISTANT LOOP (Fixed-Response Version with Night Findings)
##############################################################################
def voice_assistant_loop(stop_event):
    ...
    while not stop_event.is_set():
        ...
        # If modes were changed, update the GUI checkbuttons (if available)
        if dashboard_gui:
...
class SecurityDashboardGUI:
    def __init__(self, root, user_data):
        ...
        # Upper 70% for Narada Log (approx. 245 pixels)
        ...
        # Lower 30% for Responses (approx. 105 pixels) with white bg and grey text
