...
import os, sys, argparse, numpy as np, setproctitle, cv2, time, smtplib, datetime
...
# ---------------------------------------------------------------------------
# Dummy implementations to replace hailo_rpi_common (so we don’t use it)
# ---------------------------------------------------------------------------
...
def get_caps_from_pad(pad):
    # Dummy: always return RGB format and 640x640 dimensions
    return ("RGB", 640, 640)

def get_numpy_from_buffer(buffer, format_, width, height):
    # Dummy: return a black image
    return np.zeros((height, width, 3), dtype=np.uint8)

class DummyAppCallback:
    def __init__(self):
        self.count = 0
        self.use_frame = True
    def increment(self):
...
class DummyGStreamerApp:
    def __init__(self, args, user_data):
        ...
        self.sync = False
        self.options_menu = type("Dummy", (), {"show_fps": True})()
        self.pipeline = None
    def create_pipeline(self):
        self.pipeline = type("DummyPipeline", (), {"set_state": lambda self, state: None})()
    def run(self):
        # Dummy pipeline loop
        while True:
            time.sleep(5)

# Alias our dummy implementations to the names used in the original code.
app_callback_class = DummyAppCallback
GStreamerApp = DummyGStreamerApp

# ---------------------------------------------------------------------------
# GLOBAL VARIABLES, LOGGING, AND HELPER FUNCTIONS
# ---------------------------------------------------------------------------
app = None
...
# Mode flags (default: all alerts enabled)
...
# Global logs
system_updates_log = []    # System updates and events
voice_assistant_log = []   # Narada log messages (listening, recognized phrases, etc.)
voice_responses = []       # Narada response messages

# This variable will be set to the GUI instance so that the voice loop can update checkbuttons.
dashboard_gui = None
...
# ---------------------------------------------------------------------------
# PIPELINE & DETECTION CODE (Integrated in this file)
# ---------------------------------------------------------------------------
...
# ---------- Pipeline Class (Dummy Hailo/GStreamer Pipeline) ----------
class GStreamerDetectionApp(GStreamerApp):
    def __init__(self, args, user_data):
        ...
        if os.path.exists(new_postprocess_path):
            ...
            self.default_postprocess_so = ""
        if args.hef_path is not None:
            self.hef_path = args.hef_path
        elif args.network == "yolov8s":
            self.hef_path = "../resources/yolov8s_h8l.hef"
        elif args.network == "yolov6n":
            self.hef_path = "../resources/yolov6n.hef"
        elif args.network == "yolox_s_leaky":
            self.hef_path = "../resources/yolox_s_leaky_h8l_mz.hef"
        else:
...
# ----------------------- USER DASHBOARD (Narada Interface) -----------------------
...
class UserDashboard(tk.Frame):
    ...
    def build_narada_tab(self):
        ...
        # Narada container (50% of tab height)
        ...
        # Upper 70% for Narada Log with Scrollbar
        ...
        # Lower 30% for Responses with Scrollbar
    ...
    def toggle_voice_assistant(self):
        if not self.voice_assistant_running:
            ...
            from __main__ import append_voice_log
    ...
    def clear_narada_log(self):
        from __main__ import voice_assistant_log, voice_responses, append_voice_log
    ...
    def update_narada_tab(self):
        from __main__ import voice_assistant_log, voice_responses
    ...
    def update_gui(self):
        ...
        from __main__ import system_updates_log
...
if __name__ == "__main__":
    ...
    parser.add_argument("--network", default="yolov8s", choices=["yolov6n", "yolov8s", "yolox_s_leaky"], help="Which Network to use (default: yolov8s)")
