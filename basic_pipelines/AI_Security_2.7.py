...
MODE_NIGHT = False       # High priority mode: alerts last 10 sec, and email subject is modified

# Global lists to store logs/responses for Narada
voice_assistant_log = []   # General log from the voice assistant loop
voice_responses = []       # Stores only the responses generated by Narada
...
def append_voice_response(message):
    """Append a timestamped entry to the voice responses log."""
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def app_callback(pad, info, user_data):
    ...
    for d in detections:
        ...
        if label == user_data.danger_label:
            # Danger detected => trigger alerts (which now check modes)
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if "your name" in command:
                if "what" in command:
                    response = "My name is Narada, your vigilant security assistant."
            elif "weather" in command:
                response = "It appears clear outside, though I cannot fetch live weather data at the moment."
                ...
                MODE_EMAIL_OFF = True
                response = "Email notifications have been turned off."
            elif "deactivate email" in command and "off" in command:
                MODE_EMAIL_OFF = False
                response = "Email notifications are now active."
                ...
                response = (f"Current modes - DND: {MODE_DND}, Email Off: {MODE_EMAIL_OFF}, "
                            f"Idle: {MODE_IDLE}, Night Mode: {MODE_NIGHT}")
            else:
...
class SecurityDashboardGUI:
    def __init__(self, root, user_data):
        ...
        # --- Modes Section (Moved to AI Detections Tab) ---
        ...
        # --- Narada Log Section ---
        self.narada_log_frame = ttk.LabelFrame(self.narada_tab, text="Narada Log")
        self.narada_log_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        self.voice_text = tk.Text(self.narada_log_frame, height=8, wrap="word", bg="lightyellow")
        self.voice_text.configure(state="disabled")
        self.voice_text.pack(fill=tk.BOTH, expand=True)

        # --- Responses Section (White background, Grey text) ---
        self.response_frame = ttk.LabelFrame(self.narada_tab, text="Responses")
        self.response_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        self.response_text = tk.Text(self.response_frame, height=8, wrap="word", bg="white", fg="grey")
        self.response_text.configure(state="disabled")
        self.response_text.pack(fill=tk.BOTH, expand=True)

        # --- System Data Terminal Section ---
        self.system_data_frame = ttk.LabelFrame(self.narada_tab, text="System Data Terminal")
        self.system_data_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)
        self.system_text = tk.Text(self.system_data_frame, height=8, wrap="word", bg="lightblue")
        ...
        # Voice Assistant thread and stop event initialization for Narada
    ...
    def update_gui(self):
        ...
        # Update Narada tab (log, responses, system terminal)
...
if __name__ == "__main__":
    ...
    # Note: Narada (voice assistant) is started/stopped via the GUI button.
