...
def send_email_alert():
    ...
    try:
        ...
        print(f"Email alert error: {str(e)}")  # Debug printsystem_update("Email alert skipped (EmailOff/Idle).")
        return
...
class AdminDashboardGUI:
    ...
    def build_logs_page(self):
        ...
        self.log_info_text = ModernLabel(
            info_bar, 
            text=f"Logs are automatically retained for {LOG_RETENTION_DAYS} days. Older entries are purged on startup.",
    ...
    def create_builtin_commands_tab(self):
        ...
        enhanced_commands = {
            **BUILT_IN_COMMANDS,
            "system status": "Get current system mode status",
            "detection count": "Get total detection statistics",
            "last detection": "When was the last detection",
            "alert summary": "24-hour alert analytics",
            "quiet hours": "Check quiet hours status",
            "all modes off": "Deactivate all modes at once",
            "quick check": "System health check",
            "security high": "Set high security mode",
            "security low": "Set low security mode"
        }
        
        for phrase, desc in enhanced_commands.items():
    ...
    def build_modes_page(self):
        ...
        # Existing modes list
        modes_frame = ModernFrame(modes_panel, bg=COLORS["bg_secondary"])
        modes_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        
        scrollbar = tk.Scrollbar(modes_frame)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        
        self.custom_modes_listbox = tk.Listbox(
            modes_frame, yscrollcommand=scrollbar.set,
            bg=COLORS["bg_primary"], fg=COLORS["text_primary"],
            font=("Arial", 10), bd=0, highlightthickness=0
        )
        ...
        delete_btn = ModernButton(modes_panel, text="Delete Selected Mode", command=self.delete_mode, style="danger")
    ...
    def update_gui(self):
        ...
        if psutil and self.pages["hardware"].winfo_ismapped():
            ...
            self.label_cpu.config(text=f"CPU Usage: {cpu}% {cpu_bar}")
            self.label_mem.config(text=f"Memory Usage: {mem}% {mem_bar}")
            self.label_disk.config(text=f"Disk Usage: {disk}% {disk_bar}")
    ...
    def _usage_bar(self, value, length=30):
        """Generate a text-based usage bar for CPU/mem/disk usage."""
    ...
    def show_instructions(self):
        instructions = (
            ...
            "Logs & Monitoring - View system logs (retained for 7 days)\n"
            "Hardware Status - Monitor CPU, memory, disk usage and GPIO states\n"
            "Narada Commands - Configure custom voice commands\n"
...
class UserLoginScreen:
    ...
    def forgot_password(self):
        ...
        if send_otp_via_email(EMAIL_SENDER, USER_FORGOT_OTP):
            ...
            messagebox.showerror("Error", "Failed to send OTP. Please check email configuration.")
...
if __name__ == "__main__":
    ...
    print(f"  Password Length: {len(EMAIL_SENDER_PASS)} chars")
