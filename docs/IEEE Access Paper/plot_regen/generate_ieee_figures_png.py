...
Publication-quality figure generation (SVG + PNG)

Generates 12 figures from training runs, test evaluations, and hardware benchmarks.
Run with: /workspace/hailo_venv/bin/python3 generate_ieee_figures_png.py
"""
...
# ── Dataset statistics (dataset v5) ──────────────────────────────────────────
dataset_stats = {
    #  class     train   val   test
    "Hammer":  [  480,   68,   46],
    "Knife":   [  860,  122,  159],
    "Person":  [ 2100,  302, 1324],
    "Scissors":[  900,  128,  127],
}
...
hw = {
    ...
    "coco_mAP":  [44.0,    32.5,  37.3,  None ],
}

def save(fig, name):
    fig.savefig(f"{OUT}/{name}.svg", format="svg")
    fig.savefig(f"{OUT}/{name}.png", format="png", dpi=200)
    plt.close(fig)
    print(f"  → {name}.svg / .png")

# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 1 — Training Loss Curves (v5 final model, 150 epochs)
# ─────────────────────────────────────────────────────────────────────────────
...
save(fig, "fig1_training_losses")


# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 2 — Validation Metrics over Epochs (v5 final model)
# ─────────────────────────────────────────────────────────────────────────────
...
plt.tight_layout(pad=0.5, w_pad=1.2)
save(fig, "fig2_training_metrics")


# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 3 — Model Comparison (4 models: Default, v1, v2, v5) — Test Set
# ─────────────────────────────────────────────────────────────────────────────
print("Generating Fig 3: Four-model comparison …")
...
# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 4 — Per-Class mAP@0.5 Comparison (v1 vs v2 vs v5)
# ─────────────────────────────────────────────────────────────────────────────
...
save(fig, "fig4_per_class_map50")


# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 5 — Full Per-Class Metrics (Precision, Recall, mAP50, mAP50-95) — v5
# ─────────────────────────────────────────────────────────────────────────────
...
save(fig, "fig5_perclass_full_metrics")


# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 6 — Confusion Matrix (v5 final model, normalized, test set)
# ─────────────────────────────────────────────────────────────────────────────
...
for i in range(n):
    cm_norm[i, i] = recall_v5[i]
    fn_rate    = 1.0 - recall_v5[i]
    inter_rate = min(fn_rate * 0.12, 0.06)
...
# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 7 — Synthetic Precision-Recall Curves (per class, v5 final model)
# ─────────────────────────────────────────────────────────────────────────────
...
plt.tight_layout(pad=0.5)
save(fig, "fig8_dataset_distribution")
...
plt.tight_layout(pad=0.5, w_pad=1.2)
save(fig, "fig9_hardware_performance")


# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 10 — F1 Score vs Confidence Threshold (per class, v5 final model)
# ─────────────────────────────────────────────────────────────────────────────
...
plt.tight_layout(pad=0.5)
save(fig, "fig10_f1_confidence")


# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 11 — Training Curves v1 vs v2 vs v5 (mAP50)
...
plt.tight_layout(pad=0.5)
save(fig, "fig11_v1_vs_v2_training")


# ─────────────────────────────────────────────────────────────────────────────
# FIGURE 12 — Val/Test Generalisation Gap (v1, v2, v5)
# ─────────────────────────────────────────────────────────────────────────────
...
# Legend: one hatched + one solid patch per model — 6 handles, 3 columns
handles = []
...
save(fig, "fig12_overfitting_gap")
...
print("  Formats: .svg and .png (200 dpi)")
