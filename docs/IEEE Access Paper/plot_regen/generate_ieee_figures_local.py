...
axes[1].set_title("Training Loss Convergence — YOLOv8s v5 (Final), 150 Epochs", pad=4, fontsize=9)

plt.tight_layout(pad=0.5, w_pad=1.2)
fig.savefig(f"{OUT}/fig1_training_losses.svg", format="svg")
fig.savefig(f"{OUT}/fig1_training_losses.pdf", format="pdf")
plt.close(fig)
...
fig.savefig(f"{OUT}/fig2_training_metrics.pdf", format="pdf")
plt.close(fig)
...
ax.set_title("Model Performance Progression on Held-Out Test Set — Default → v1 → v2 → v5", pad=4)
...
fig.savefig(f"{OUT}/fig3_model_comparison.pdf", format="pdf")
plt.close(fig)
...
ax.set_title("Per-Class mAP@0.5 — v1 vs v2 vs v5 (Test Set)", pad=4)

plt.tight_layout(pad=0.5)
fig.subplots_adjust(bottom=0.20)
fig.savefig(f"{OUT}/fig4_per_class_map50.svg", format="svg")
fig.savefig(f"{OUT}/fig4_per_class_map50.pdf", format="pdf")
plt.close(fig)
...
ax.set_title("Fine-tuned v5 (Final) — Per-Class Detection Performance (Test Set)", pad=4)

plt.tight_layout(pad=0.5)
fig.subplots_adjust(bottom=0.18)
fig.savefig(f"{OUT}/fig5_perclass_full_metrics.svg", format="svg")
fig.savefig(f"{OUT}/fig5_perclass_full_metrics.pdf", format="pdf")
plt.close(fig)
...
fig.savefig(f"{OUT}/fig6_confusion_matrix.pdf", format="pdf")
plt.close(fig)
...
fig.savefig(f"{OUT}/fig7_pr_curves.pdf", format="pdf")
plt.close(fig)
...
ax.set_title(
    f"Dataset Split Distribution by Class (Dataset v5)\n"
...
fig.savefig(f"{OUT}/fig8_dataset_distribution.pdf", format="pdf")
plt.close(fig)
...
fig.savefig(f"{OUT}/fig9_hardware_performance.pdf", format="pdf")
plt.close(fig)
...
ax.set_title("F1–Confidence Threshold\nFine-tuned v5 (Final) — Test Set", pad=4)

plt.tight_layout(pad=0.5)
fig.savefig(f"{OUT}/fig10_f1_confidence.svg", format="svg")
fig.savefig(f"{OUT}/fig10_f1_confidence.pdf", format="pdf")
plt.close(fig)
...
ax.set_title("Validation mAP@0.5 — v1 vs v2 vs v5 (Effect of Dataset Size)", pad=4)

plt.tight_layout(pad=0.5)
fig.savefig(f"{OUT}/fig11_v1_vs_v2_training.svg", format="svg")
fig.savefig(f"{OUT}/fig11_v1_vs_v2_training.pdf", format="pdf")
plt.close(fig)
...
ax.yaxis.set_minor_locator(MultipleLocator(0.05))
ax.grid(True, which="minor", linewidth=0.25, alpha=0.25)
ax.set_title("Val vs Test mAP@0.5 Generalisation Gap — v1, v2, v5", pad=4)

plt.tight_layout(pad=0.5)
fig.subplots_adjust(bottom=0.26)
fig.savefig(f"{OUT}/fig12_overfitting_gap.svg", format="svg")
fig.savefig(f"{OUT}/fig12_overfitting_gap.pdf", format="pdf")
plt.close(fig)
