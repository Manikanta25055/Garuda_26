...
\begin{abstract}
Home security products are split between two inadequate deployment models: cloud-backed systems that export every frame to a data centre and charge a subscription, and do-it-yourself (DIY) single-board-computer builds that are typically limited to frame-differencing motion detection. No consumer-class device combines edge inference, encrypted evidence capture, remote access, voice control, and tamper detection. This paper presents Project Garuda, a privacy-first AI home security system on a Raspberry Pi~5 with a Hailo-8L AI HAT+ (13~TOPS, INT8). A YOLOv8s detector, fine-tuned on a 4{,}982-image dataset across four security-relevant classes (Hammer, Knife, Person, Scissors) and compiled to an INT8 HEF, runs through a GStreamer TAPPAS pipeline at a sustained 52.2~FPS with 18.4~ms NPU latency and reaches a held-out test mAP{@}0.5 of 0.847. A three-stage verification cascade adds MobileNetV2 weapon classification and MiDaS Small depth-based anti-spoof when engaged. The system layer integrates twelve cooperating engines --- detection, alert, voice (Narada, with a Groq large-language-model fallback bounded to a stateless free-tier text-only API), mode control, address resolution protocol (ARP) presence monitoring, a FastAPI web server behind a Cloudflare tunnel, password-based key derivation function~2 (PBKDF2-SHA256) authentication with one-time-password (OTP) two-factor verification, AES-256-CBC evidence encryption, a 180-second deadman watchdog, and a SQLite-backed offline event log. Video never leaves the device during inference. The total bill of materials is US\$450--500. We report training convergence, per-class metrics, confusion structure, and INT8 quantisation impact. Extensions to low-light operation via the NoIR camera are left to future work.
\end{abstract}
...
\begin{enumerate}
\item A custom-fine-tuned YOLOv8s detector for four security-relevant classes (Hammer, Knife, Person, Scissors), trained on a 4{,}982-image dataset assembled in three passes and compiled to an INT8 HEF, reaching a test mAP{@}0.5 of 0.847 at a sustained 52.2~FPS on the Hailo-8L.
\item A three-stage cascade architecture (YOLOv8s + MobileNetV2 classifier + MiDaS Small depth estimator) that adds weapon verification and photo/screen spoof rejection, with all three networks multiplexed on a single VDevice.
\item A modular architecture composed of twelve cooperating engines (hardware, GStreamer, detection, cascade, alert, voice, mode, presence, web, authentication, deadman, logging), integrated under a single FastAPI application and exposed through a Cloudflare tunnel, delivering a complete security platform with no cloud inference dependency.
...
The block diagram separates the system into three horizontal layers. The inference pipeline layer at the top carries video from the camera through the GStreamer TAPPAS pipeline into the Hailo-8L NPU, which runs YOLOv8s and emits watch or danger classifications. The response and control layer in the middle contains the alert engine, Narada, the mode controller, and the presence monitor; these are the components driven by detection events, and they decide what actions are taken based on the current operating mode. The web server and persistence layer at the bottom handles everything that faces outward: the FastAPI application, the Cloudflare tunnel, PBKDF2 authentication, the deadman watchdog, and the multi-tier logging engine. Every engine in the upper layers writes events into this bottom layer for storage and client delivery.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.9\textwidth,keepaspectratio]{garuda_block_diagram.drawio-3.pdf}
\caption{Three-layer block diagram of Project Garuda. Top: inference pipeline (camera $\to$ GStreamer $\to$ Hailo-8L $\to$ detection classes). Middle: response and control (alert engine, Narada voice assistant, mode controller, presence monitor). Bottom: persistence and external communication (FastAPI + Cloudflare tunnel, authentication, deadman watchdog, multi-tier logging).}
...
\caption{Project Garuda end-to-end flowchart. Video enters the GStreamer TAPPAS pipeline; YOLOv8s runs on the Hailo-8L NPU; post-inference detections are routed through watch/danger classification and a mode gate that fans out into email, WebSocket, and encrypted clip upload. Parallel subsystems (voice assistant, presence monitor, deadman watchdog, mode scheduler, authentication, and the Cloudflare-tunnelled web server) execute concurrently and feed the multi-tier logging engine.}
...
Building on the system overview presented in Section~\ref{sec:overview}, this section describes each engine in detail, in the order in which video and control flow move through the system: hardware first, then the GStreamer pipeline and the detection callback, the three-stage cascade, the alert path, the voice assistant, the mode controller, the presence monitor, the web server, authentication, the deadman watchdog, and finally the logging subsystem. Each engine receives a functional description, an implementation note, a figure that captures its control flow, and any tables that fix the numeric parameters.
...
VDevice multiplexing is the mechanism that allows all three networks to share a single 13~TOPS budget without contention. The three HEFs are loaded onto one VDevice at startup and the inference thread issues sequential \texttt{infer} calls; because MobileNetV2 and MiDaS both run at 200--500+~FPS on this chip, the combined cost per Person crop is under 4~ms --- well inside the 19.2~ms frame budget implied by the baseline 52~FPS target. The anti-spoof decision is grounded in physical geometry: a flat panel (photo, screen) produces a normalised-depth variance $\sigma^2 < 0.005$ in measurements, while a real person at arm's length produces $\sigma^2 \in [0.022, 0.10]$, giving a $4\times$ safety margin for the 0.05 threshold.
...
Person + weapon verification & YOLOv8s + MobileNetV2 & 0.647 + 99.6\% cls. & 52.2 & 24\,ms & Cascade verdict \\
...
Privacy mode is enforced inline within the alert engine. When Privacy is active, a Gaussian blur with kernel size $k=51$~px and standard deviation $\sigma=30$~px is applied to every person bounding box before the MJPEG/WebRTC publisher emits the frame. Stored clips and remote streams therefore never contain an identifiable subject, irrespective of who is viewing the stream. The alert flow is summarised in Fig.~\ref{fig:alert}.
...
\hline
Hammer   & $\sim$39   & $\sim$7   & 0.848 \\
Knife    & $\sim$130  & $\sim$29  & 0.817 \\
Person   & $\sim$807  & $\sim$517 & 0.609 \\
Scissors & $\sim$118  & $\sim$9   & 0.929 \\
\hline
\textbf{Overall} & \textbf{\textasciitilde 1{,}094} & \textbf{\textasciitilde 562} & \textbf{0.801} \\
\hline
\end{tabular}
\end{table}
...
FPS                  & $\sim$450 (batch) & 52.2        & NPU-limited \\
...
\textbf{Garuda}         & \textbf{RPi~5 + Hailo-8L} & \textbf{52.2} & \textbf{4-class} & \textbf{MiDaS depth} \\
\hline
\end{tabular}%
}
\end{table}
...
\subsection{Models}
Three neural networks are deployed. The primary detector is a YOLOv8s model fine-tuned on the custom four-class dataset described in Section~\ref{sec:dataset} and post-training-quantised to INT8 for the Hailo-8L. The second model is a MobileNetV2 classifier that runs on cropped person regions to produce a Safe / Weapon label. The third is MiDaS Small, used as a monocular anti-spoof stage that rejects flat photos and screen replays via depth-variance analysis. All three are summarised in Table~\ref{tab:model_specs}, and the deployed HEF files are listed in Table~\ref{tab:hef_files}.
...
\hline
\texttt{best\_v5.hef}                & 22.9~MB & Object detection   & 52.2 & 18.4~ms \\
