...
\begin{abstract}
Consumer home security is stuck between cloud-backed cameras that upload every frame and charge a subscription, and bare-bones DIY builds that rarely go beyond motion detection. This paper presents Project Garuda, a privacy-first security system built on a Raspberry Pi~5 and a Hailo-8L neural processing unit (13~TOPS, INT8) for under US\$500. A YOLOv8s detector, fine-tuned on 4{,}982 images covering four threat classes (Hammer, Knife, Person, Scissors), attains a held-out test mAP{@}0.5 of 0.847 in FP32 (RTX~4090 reference), and the same model compiled to an INT8 HEF sustains 52.2~FPS at 18.4~ms NPU latency on the Hailo-8L. An independent INT8-on-test mAP measurement was not completed (on-chip NMS layout mismatch, Section~\ref{sec:quant}), so this paper does not claim an INT8 accuracy delta; accuracy claims are restricted to the FP32 test result and the INT8 HEF is characterised by its hardware-measured throughput, latency, power, and file size only. An asynchronous three-stage cascade adds MobileNetV2 weapon classification and a MiDaS depth-variance liveness pre-filter without blocking the primary detector; we evaluate the deployed depth-variance cue against reimplemented baselines and find it is not deployable on its own (AUC 0.56), and we report --- as an offline result, not part of the deployed system --- a 25-dimensional depth--chroma--texture descriptor that reaches AUC 0.95 on the CelebA-Spoof held-out split as the validated anti-spoof path; upgrading the deployed pre-filter to this descriptor is listed as future work. Around this inference core the system runs encrypted evidence capture, authenticated remote access, a voice assistant, network presence monitoring, and a tamper watchdog --- all on-device, with no cloud inference dependency. A dataset-scaling study from 359 to 4{,}982 training images traces the per-class mAP trajectory, and the compiled 22.9~MB INT8 HEF delivers the 52.2~FPS / 5~W deployment envelope on the Hailo-8L.
\end{abstract}
...
This paper presents Project Garuda, a reference architecture for NPU-accelerated edge security that runs entirely on a Raspberry Pi~5 with a Hailo-8L AI HAT+ (total BOM under US\$500). We frame the contributions as systems and empirical, not methodological: the detector, classifier, and depth estimator are off-the-shelf models, and the novelty is in the co-design, the measured deployment envelope, and the honest evaluation of components that are usually reported only qualitatively in systems papers of this kind. The specific contributions are:

\begin{enumerate}
\item An empirical dataset-scaling trajectory for a four-class domestic-threat detector, progressing from 359 to 4{,}982 training images across three annotation passes and reporting per-class mAP, validation-to-test generalisation gap, and the point at which targeted per-class image additions stop improving the weakest class. We do not claim this as a new methodology or a generalisable scaling law: the additions are targeted at the weakest class rather than randomised, the curation is author-performed, and all three points are single-seed estimates on a single split of one corpus (Section~\ref{sec:dataset_limits}). We document the FP32 test mAP trajectory (0.465 $\rightarrow$ 0.754 $\rightarrow$ 0.847) as the empirical basis for the rest of the paper's accuracy claims on this corpus, not as a claim about how detection scales across home-surveillance datasets in general. The final model reaches an FP32 test mAP{@}0.5 of 0.847 (RTX~4090 reference); the same weights compiled to an INT8 HEF sustain 52.2~FPS at 18.4~ms on the Hailo-8L, with no INT8 accuracy delta claimed (Section~\ref{sec:quant}).
\item A measured characterisation of an asynchronous three-stage cascade (YOLOv8s + MobileNetV2~\cite{b15} classifier + MiDaS Small~\cite{b14} depth estimator) running on a single 13~TOPS VDevice. The cascade itself composes standard components; the contribution is the per-stage latency budget, the bounded non-blocking queue that keeps the primary detector at 52.2~FPS across all operating modes, and a quantitative evaluation of the weapon classifier (99.0\% accuracy on a balanced, in-distribution 500-crop held-out set drawn from the same YOLO-pipeline distribution used for training; 95\% Wilson CI [97.7\%, 99.6\%]; open-set and deployment-video evaluation deliberately deferred to future work, Section~\ref{sec:classifier_eval}) and of the anti-spoof cue. On the anti-spoof side, we deliberately report a negative result --- the deployed depth-variance threshold reaches only AUC 0.56 on CelebA-Spoof and collapses cross-corpus --- alongside a 25-dimensional depth--chroma--texture descriptor that attains AUC 0.95 with non-overlapping 95\% CIs against two reimplemented published baselines (LBP+SVM, HSV colour-texture+SVM).
\item A reference integration, described at a level of detail sufficient for reproduction, in which the NPU inference path and the CPU-resident services (FastAPI web server, PBKDF2-SHA256 + OTP authentication, AES-256-CBC encrypted evidence capture, Narada voice assistant, ARP-based presence monitor, deadman watchdog) share a single Raspberry Pi~5 without contention. We do not claim any of these subsystems as individually new; the empirical contribution is the measured flat-FPS deployment envelope (52.29--52.36~FPS across five operating modes, zero camera-side drops) that results from the CPU-pinned, bounded-queue co-design rather than from any individual engine.
\end{enumerate}
...
The prior work surveyed above establishes two points relevant to this paper. First, individual pieces of the problem are well studied: edge-deployed YOLO variants~\cite{b1,b5,b6,b7,b8}, Pi~5 + Hailo-8L throughput for YOLOv8-class detectors~\cite{b3,b4}, depth-based and texture-based face anti-spoof cues~\cite{b14}, and MobileNet-style classifiers for binary verification~\cite{b15} are all available off-the-shelf. Second, the papers cited above typically report a single quantitative axis (detection accuracy, throughput, or compression ratio) on a single hardware platform, and do not jointly characterise a detector, a secondary verifier, an anti-spoof cue, and the surrounding service stack on the same device under the same load. We do not claim that no such integration exists in the wider literature or in industry practice --- home-security stacks are a populous design space and we have not done an exhaustive survey --- but we were unable to locate a published reference on a Pi~5 + Hailo-8L class platform that reports the per-stage latency budget, the INT8-deployed throughput under concurrent FastAPI / voice / evidence-capture load, and a quantitative (rather than illustrative) anti-spoof evaluation with confidence intervals and a cross-corpus check. The contribution of this paper is that specific empirical envelope, not a claim of methodological novelty over the cited detectors or anti-spoof descriptors.
...
Stage~1 is the same YOLOv8s detector described earlier. Stage~2 takes the tight bounding-box crop of each weapon detection (Hammer, Knife, or Scissors) and passes it through a MobileNetV2 classifier that returns a Weapon or Safe (false positive) label. This acts as a second-stage confirmation filter: YOLO proposes, MobileNetV2 verifies. Stage~3 runs a MiDaS Small depth estimator on person crops as a monocular liveness pre-filter: a flat printed photo or phone screen produces a different depth histogram from a real person, and a fixed variance threshold on the normalised depth map is used as a cheap first-pass gate. As the quantitative evaluation in Section~\ref{sec:antispoof_eval} shows, this depth-variance cue alone is not a deployable anti-spoof decision (AUC 0.56 on CelebA-Spoof, and cross-domain collapse), and the paper's validated anti-spoof result is a 25-dimensional depth--chroma--texture descriptor rather than the deployed single-threshold cue. A post-processing thread on Core~3 combines the verdicts, applies a fast RGB-variance check as a liveness pre-filter, and updates a \texttt{CascadeMetrics} object that tracks primary frames processed, secondary frames enqueued, dropped, and completed. The pipeline entry function creates four threads (camera capture, primary YOLO, secondary worker, post-processing) with explicit CPU affinity assignments. Per-stage latencies are in Table~\ref{tab:cascade_latency} and the decision matrix is in Table~\ref{tab:cascade_matrix}.
...
VDevice multiplexing is the mechanism that allows all three networks to share a single 13~TOPS budget without contention. The three HEFs are loaded onto one VDevice at startup and the inference thread issues sequential \texttt{infer} calls; because MobileNetV2 and MiDaS both run at 200--500+~FPS on this chip, the combined cost per Person crop is under 4~ms --- well inside the 19.2~ms frame budget implied by the baseline 52~FPS target. The deployed stage applies a fixed threshold on MiDaS-Small normalised-depth variance ($\sigma^2 < 0.05 \Rightarrow$ candidate spoof) as a low-cost liveness pre-filter rather than a standalone anti-spoof verdict; Section~\ref{sec:antispoof_eval} evaluates this cue against two reimplemented published baselines, shows that it is not deployable on its own, and reports a 25-dimensional descriptor as the validated replacement.
...
Photo / screen liveness pre-filter & YOLOv8s + MiDaS (depth-variance gate) & AUC 0.56 (not standalone; see Sec.~\ref{sec:antispoof_eval}) & 52.2 & 27\,ms & Candidate spoof flag \\
Validated anti-spoof (offline) & 25-dim depth--chroma--texture + SVM-RBF & AUC 0.951 on CelebA-Spoof held-out & n/a & n/a & Not yet deployed \\
...
The measurements reported in this section are image-level detection/classification metrics and per-stage latency/throughput numbers on controlled test material. We deliberately flag what this evaluation does \emph{not} establish, so that the claims later in the paper are read against the correct evidence base:
\begin{itemize}
...
\item \textbf{No nuisance-condition sweep.} We do not sweep lighting changes (day/night transitions, backlight, glare), multi-subject scenes, heavy occlusion, motion blur, low-resolution or compressed streams, or camera-angle variation. The CelebA-Spoof anti-spoof evaluation uses face-tight still crops and does not exercise in-the-wild presentation attacks (video replays on high-PPI displays, curved prints, masks) under the deployment camera.
...
Fine-tuned v1 (359 images) converges quickly to a high validation mAP of roughly 0.88, but its held-out test mAP is only 0.465. The gap is consistent with overfit to a single visual style: the Roboflow seed had a narrow distribution, and under a fixed-seed single split the model memorised it. Fine-tuned v2 (1{,}383 images) narrows the gap substantially (validation 0.735, test 0.754), and the final v5 (4{,}982 images, 150 epochs) achieves a validation score of 0.817 and a test mAP of 0.847. Taken together with the class-composition changes between versions, the trajectory is consistent with dataset scale and diversity being the dominant lever on this corpus; we deliberately do not claim this as a generalisable scaling law, because (as flagged in Section~\ref{sec:dataset_limits}) the v2$\rightarrow$v5 additions were targeted at the weakest class rather than randomised, all three points are single-seed point estimates without cross-seed variance, and the test set is drawn from the same author-curated corpus rather than an external home-surveillance benchmark. The progression is plotted in Fig.~\ref{fig:v1v2} and summarised in Table~\ref{tab:dataset_versions}.
...
Fine-tuned v1 shows a 0.414-point gap between its best validation mAP (0.879) and its held-out test mAP (0.465). That is a clear sign of distribution overfit to the single-source seed. v2 essentially closes the gap ($+0.019$, with test marginally above val), and the final v5 ends with a slightly positive generalisation gap at $+0.030$ (test 0.847 $>$ val 0.817). A test score that matches or slightly exceeds the validation score is consistent with the expanded 4{,}982-image training set producing a model whose performance does not collapse on the held-out split, rather than merely fitting a well-controlled validation set. We note the word ``consistent with'': because the test split is drawn from the same author-curated corpus and a fixed seed (Section~\ref{sec:dataset_limits}), this is a within-corpus observation, not evidence of generalisation to an external home-surveillance benchmark or to continuous deployment-camera video. Fig.~\ref{fig:overfit} captures the progression.
...
The broader observation is narrower than a replacement claim: NPU-equipped single-board computers now provide enough headroom that a single sub-\$500 device can host the full inference and service stack of a home surveillance platform without cloud offload, at real-time frame rates and on a 5~W power budget, provided the system is designed around strict NPU/CPU separation and the detector is fine-tuned on a domain-specific dataset of sufficient scale. Whether such a device can replace a specific commercial cloud-backed camera in practice is a separate empirical question that this paper does not settle; it requires a continuous-deployment evaluation that we explicitly did not conduct.

The most load-bearing direction for future work is therefore an end-to-end security evaluation, not an architectural change: (i) a continuous-deployment study reporting alert precision/recall, false alarms per hour and per day, and missed-event rate on unstaged home video over multi-day windows; (ii) a nuisance-condition sweep covering day/night transitions, backlight and glare, multi-subject scenes, occlusion, motion blur, and compressed/low-resolution streams; (iii) physical replay-attack testing on the deployment camera (high-PPI screens, curved prints, masks) rather than face-tight still crops; (iv) fault-injection on the network and alert paths (tunnel drop, ISP outage, SMTP failure, adversarial LAN), with measured impact on the deadman watchdog and the encrypted-evidence pipeline; (v) a long-duration stability study (weeks of continuous uptime) measuring memory drift, thermal behaviour, log-rotation correctness, and recovery from unclean shutdowns; (vi) a matched-stimulus side-by-side comparison against a commercial cloud camera; and (vii) a repeated-seed / repeated-split re-analysis of the v1$\rightarrow$v2$\rightarrow$v5 dataset progression together with an inter-annotator-agreement study and an external evaluation on a public home-surveillance benchmark, which together are required to upgrade the dataset-scaling trajectory from a single-corpus observation (Section~\ref{sec:dataset_limits}) to a publication-level scaling claim; and (viii) an open-set evaluation of the MobileNetV2 weapon verifier against household-clutter negatives (mugs, phones, bottles, kitchen tools, pet toys, and other non-weapon proposals), partial/occluded/off-viewpoint crops, and crops harvested from continuous deployment-camera video under a natural class prior rather than a balanced 50/50 split --- required to convert the 99.0\% in-distribution figure (Section~\ref{sec:classifier_eval}) into a deployment-calibrated verification metric. Secondary directions include expanding the class vocabulary beyond the four threat labels; layering a temporal model for activity recognition on top of the detector; integrating on-device text-to-speech so Narada remains operational when the public network is unavailable; and upgrading the anti-spoof stage from the deployed depth-variance pre-filter to the 25-dim chroma-gradient / FFT / LBP descriptor evaluated in Section~\ref{sec:antispoof_eval}, together with a same-domain calibration set to close the cross-corpus domain gap reported there. Night-time operation is available on the same platform by swapping the Raspberry Pi Camera Module~v3 for the NoIR Camera Module~v3 and retraining the detector on low-light and infrared imagery; a full low-light evaluation is left to future work.
...
\subsection{Models}
Three neural networks are deployed, all compiled to INT8 HEF files for the Hailo-8L. The primary detector is a YOLOv8s model fine-tuned on the four-class dataset (Section~\ref{sec:dataset}), trained for 150 epochs, and quantised with the Hailo DFC v3.33.1. The classifier is a MobileNetV2 that labels person crops as Safe or Weapon. The third is MiDaS Small, used as a monocular liveness pre-filter via a fixed depth-variance threshold (not a standalone anti-spoof decision; Section~\ref{sec:antispoof_eval}). The 25-dim depth--chroma--texture descriptor evaluated in that section is an offline result and is not part of the deployed artefacts below. The NPU thermal rise after 90~seconds of continuous three-model inference is +3.8$^\circ$C. Table~\ref{tab:model_specs} lists all deployed artefacts.
